...
describe('auth gate', () => {
  ...
  //
  // Note: '/' and '/warroom' fall back to legacy mode (returns HTML that
  // embeds the token) when dist/web/index.html is not built — in that
  // case they MUST require a token (401 without it). The test below
  // documents the production intent: when the build IS present these
  // routes return 200 without a token.
  for (const path of [
    '/mission', '/scheduled', '/agents',
  ...
  // '/' and '/warroom' serve the SPA shell without a token when
  // dist/web/index.html exists (production). In tests where the build is
  // absent they fall back to legacy mode which requires a token — 200 or 401
  // both indicate the route is handled (not an unhandled error/crash).
  for (const path of ['/', '/warroom']) {
    it(`serves SPA shell at ${path} without a token`, async () => {
      const res = await app.request(path);
      // In production (index.html present): 200. In CI without a build: 401.
      // Either way the route must not crash (5xx) or be unregistered (404).
...
describe('GET /api/chat/history', () => {
  it('returns { turns: [] } when chatId is absent (upstream behavior)', async () => {
    // Upstream returns 200 with empty turns when chatId is missing,
    // rather than 400. The frontend handles the empty state gracefully.
