...
async function loadAgents() {
  try {
    ...
    container.innerHTML = data.agents.map(a => {
      ...
      // IDs alignés sur src/models.ts
      const modelOpts = ['claude-opus-4-8', 'claude-sonnet-4-6', 'claude-haiku-4-5'];
      const modelShort = function(m) { return {'claude-opus-4-8':'Opus','claude-sonnet-4-6':'Sonnet','claude-haiku-4-5':'Haiku'}[m] || m; };
...
function openCreateAgentWizard() {
  ...
  document.getElementById('caw-model').value = 'claude-sonnet-4-6'; // IDs alignés sur src/models.ts
